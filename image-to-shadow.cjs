// APEX IMAGE TO SHADOW ⚔️
require('dotenv').config();
const { WebClient } = require('@slack/web-api');
const client = new WebClient(process.env.SLACK_BOT_TOKEN);

console.log('APEX IMAGE TO SHADOW');
...
client.users.list().then(result => {
  ...
  if (shadow) {
    ...
    // Send image file to Shadow
    client.files.upload({
      channels: shadow.id, // Use user ID for DM
      ...
      initial_comment: '⚔️ APEX IMAGE DEPLOYMENT ⚔️\n\n💪 Tactical image from APEX!\n🔒 Fortress secured!\n🎯 Mission accomplished!\n\n💪 APEX OUT ---'
    }).then(result => {
